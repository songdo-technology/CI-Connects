...
// ------------------------------------------------------------------ the back
const TIER_HEADING: Record<SponsorTier, string> = { platinum: 'Platinum', gold: 'Gold', silver: 'Silver', exhibitor: 'Exhibitors' };
/** How tall a sponsor's mark is: the higher the tier and the fewer there are, the larger. */
const markHeight = (tier: SponsorTier, n: number) =>
  tier === 'platinum' ? (n <= 1 ? 64 : n === 2 ? 50 : 40) : tier === 'gold' ? (n <= 2 ? 40 : 32) : tier === 'silver' ? (n <= 3 ? 30 : 24) : 22;

const SponsorMark: React.FC<{ sponsor: Sponsor; height: number }> = ({ sponsor, height }) => sponsor.logoUrl
...
const BadgeBack: React.FC<{ event: Event; sponsors: Sponsor[]; onFlip: () => void; full: boolean }> = ({ event, sponsors, onFlip, full }) => {
  ...
  return (
    <div className="relative h-full flex flex-col rounded-[1.75rem] overflow-hidden bg-white border border-sand-200 shadow-[var(--shadow-pop)]">
      ...
      <div className={`relative bg-blue-900 text-white overflow-hidden ${full ? 'px-6 pt-8 pb-4' : 'px-5 pt-7 pb-3'}`}>
        ...
        {groups.map(([tier, list]) => (
          <section key={tier} className="text-center">
            ...
            <div className="flex flex-wrap items-center justify-center gap-x-4 gap-y-2">
              {list.map((s) => <SponsorMark key={s.id} sponsor={s} height={Math.round(markHeight(tier, list.length) * (full ? 1 : 0.82))} />)}
            </div>
          </section>
        ))}
      </div>
