import { initialsAvatar } from '../lib/avatar';
import { Room, Track, Sponsor, UserProfile, Session, CommunityTopic, BroadcastAnnouncement, MealService, AttendanceRecord, DirectMessage, EventConfig, FeedbackEntry } from '../types';

export const INITIAL_TRACKS: Track[] = [
  // The five strands of the Chadwick mission: to develop global citizens with
  // keen minds, exemplary character, self-knowledge and the ability to lead.
  // Colors are Chadwick secondaries, deepened to clear WCAG AA behind white.
  { id: 'track-global', name: 'Global Citizenship', colorHex: '#002B54', orderIndex: 1 },
  { id: 'track-minds', name: 'Keen Minds', colorHex: '#2A6791', orderIndex: 2 },
  { id: 'track-character', name: 'Exemplary Character', colorHex: '#5E6513', orderIndex: 3 },
  { id: 'track-self', name: 'Self-Knowledge', colorHex: '#6B605A', orderIndex: 4 },
  { id: 'track-lead', name: 'Leadership', colorHex: '#B04318', orderIndex: 5 },
];

export const INITIAL_ROOMS: Room[] = [
  { id: 'room-theater', name: 'Main Theater', capacity: 600, floorLabel: 'Level 1', building: 'Performing Arts Center' },
  { id: 'room-blackbox', name: 'Black Box', capacity: 200, floorLabel: 'Level 1', building: 'Performing Arts Center' },
  { id: 'room-cihub', name: 'Collaboration and Innovation Hub', capacity: 40, floorLabel: 'Level 2', building: 'Library' },
  { id: 'room-b201', name: 'B-201', capacity: 25, floorLabel: 'Level 2', building: 'B Building' },
  { id: 'room-b202', name: 'B-202', capacity: 25, floorLabel: 'Level 2', building: 'B Building' },
  { id: 'room-b203', name: 'B-203', capacity: 25, floorLabel: 'Level 2', building: 'B Building' },
];

export const INITIAL_SPONSORS: Sponsor[] = [
  {
    id: 'spon-chadwick',
    name: 'Chadwick International',
    tier: 'Host',
    tagline: 'Conference Host',
    description: 'Convening the conference on campus in Songdo.',
  },
  {
    id: 'spon-mcgraw',
    name: 'McGraw Hill Education',
    tier: 'Platinum',
    isPremier: true,
    websiteUrl: 'https://www.mheducation.com',
    tagline: 'Premier Platinum Sponsor',
    description: 'Learning science and curriculum resources for schools worldwide.',
  },
  {
    id: 'spon-incheon',
    name: 'Incheon Tourism Organization',
    tier: 'Platinum',
    tagline: 'Platinum Sponsor - Host City Partner',
    description: 'Supporting international gatherings across the Incheon region.',
  },
  {
    id: 'spon-rustic',
    name: 'Rustic Pathways',
    tier: 'Platinum',
    tagline: 'Platinum Sponsor - Global Programmes',
    description: 'Student travel and service learning programmes worldwide.',
  },
  {
    id: 'spon-britannica',
    name: 'Britannica Education',
    tier: 'Gold',
    tagline: 'Gold Sponsor',
  },
  {
    id: 'spon-cre8te',
    name: 'Cre8te',
    tier: 'Gold',
    tagline: 'Gold Sponsor',
  },
  {
    id: 'spon-creverse',
    name: 'Creverse',
    tier: 'Gold',
    tagline: 'Gold Sponsor',
  },
  {
    id: 'spon-deahn',
    name: 'DeAhn',
    tier: 'Gold',
    tagline: 'Gold Sponsor',
  },
  {
    id: 'spon-epocha',
    name: 'Epocha',
    tier: 'Gold',
    tagline: 'Gold Sponsor',
  },
  {
    id: 'spon-faria',
    name: 'Faria',
    tier: 'Gold',
    tagline: 'Gold Sponsor',
  },
  {
    id: 'spon-poin2lab',
    name: 'Poin2Lab',
    tier: 'Gold',
    tagline: 'Gold Sponsor',
  },
  {
    id: 'spon-tebahsoft',
    name: 'Tebahsoft',
    tier: 'Gold',
    tagline: 'Gold Sponsor',
  },
  {
    id: 'spon-tiecare',
    name: 'TieCare International',
    tier: 'Gold',
    tagline: 'Gold Sponsor',
  },
  {
    id: 'spon-upgrad',
    name: 'upGrad',
    tier: 'Gold',
    tagline: 'Gold Sponsor',
  },
  {
    id: 'spon-silver-open-1',
    name: 'Your organisation here',
    tier: 'Silver',
    isPlaceholder: true,
    tagline: 'Silver tier - available',
  },
  {
    id: 'spon-silver-open-2',
    name: 'Your organisation here',
    tier: 'Silver',
    isPlaceholder: true,
    tagline: 'Silver tier - available',
  },
  {
    id: 'spon-edevents',
    name: 'Ed Events',
    tier: 'Bronze',
    tagline: 'Bronze Sponsor',
  },
  {
    id: 'spon-scalaedu',
    name: 'Scalaedu',
    tier: 'Bronze',
    tagline: 'Bronze Sponsor',
  },
  {
    id: 'spon-bronze-open',
    name: 'Your organisation here',
    tier: 'Bronze',
    isPlaceholder: true,
    tagline: 'Bronze tier - available',
  },
  {
    id: 'spon-cambridge',
    name: 'Cambridge University Press & Assessment',
    tier: 'Exhibitor',
    tagline: 'Exhibitor',
  },
  {
    id: 'spon-finalsite',
    name: 'Finalsite',
    tier: 'Exhibitor',
    tagline: 'Exhibitor',
  },
  {
    id: 'spon-hachette',
    name: 'Hachette Learning',
    tier: 'Exhibitor',
    tagline: 'Exhibitor',
  },
  {
    id: 'spon-nwea',
    name: 'NWEA',
    tier: 'Exhibitor',
    tagline: 'Exhibitor',
  },
  {
    id: 'spon-shelterbox',
    name: 'ShelterBox',
    tier: 'Exhibitor',
    tagline: 'Exhibitor',
  },
  {
    id: 'spon-toefl',
    name: 'TOEFL',
    tier: 'Exhibitor',
    tagline: 'Exhibitor',
  },
  {
    id: 'spon-viva',
    name: 'Viva',
    tier: 'Exhibitor',
    tagline: 'Exhibitor',
  },
];

export const INITIAL_PROFILES: UserProfile[] = [
  {
    id: 'usr-ted-hill',
    email: 'thill@chadwickschool.org',
    fullName: 'Ted Hill',
    title: 'Head of School',
    department: "Head of School's Office",
    organization: 'Chadwick International',
    userType: 'internal_faculty',
    role: 'speaker',
    avatarUrl: initialsAvatar('Ted Hill'),
    bio: 'Head of School at Chadwick International and a mission-driven pioneer, carrying forward the torch lit by Margaret Chadwick. His leadership returns again and again to her conviction that a school exists to develop the whole person - and that the Chadwick Way is not a slogan but a set of daily choices about how we treat one another.',
    isDirectoryVisible: true,
    checkedIn: true,
    checkedInAt: 'Oct 15, 2027 07:50 AM',
    dietaryTag: 'western',
    shareContactOnScan: true,
    interests: ['Mission & Values', 'The Chadwick Way', 'School Leadership'],
  },
  {
    id: 'usr-sarah-lin',
    email: 'slin@chadwickschool.org',
    fullName: 'Dr. Sarah Lin',
    title: 'Computer Science Department Chair',
    department: 'STEM & Computer Science',
    organization: 'Chadwick International',
    userType: 'internal_faculty',
    role: 'attendee',
    avatarUrl: initialsAvatar('Dr. Sarah Lin'),
    bio: 'Pioneering algorithmic thinking and machine learning curricula across K-12 classrooms. Researching student AI agency.',
    isDirectoryVisible: true,
    checkedIn: true,
    checkedInAt: 'Oct 15, 2027 08:15 AM',
    linkedInUrl: 'https://www.linkedin.com/in/sarah-lin-cs',
    dietaryTag: 'vegetarian',
    shareContactOnScan: true,
    isPlaceholder: true,
    interests: ['AI in Education', 'Curriculum Design', 'Robotics'],
  },
  {
    id: 'usr-marcus-vance',
    email: 'marcus@vance-systems.io',
    fullName: 'Marcus Vance',
    title: 'Principal Distributed Systems Architect',
    department: 'Enterprise Architecture',
    organization: 'Vance Systems & EdTech Alliance',
    userType: 'external_guest',
    role: 'speaker',
    avatarUrl: initialsAvatar('Marcus Vance'),
    bio: 'Sample speaker entry. Replace with the visiting practitioner presenting this session - a short biography of two or three sentences works best on the public page and the printed badge.',
    isDirectoryVisible: true,
    checkedIn: true,
    checkedInAt: 'Oct 15, 2027 08:02 AM',
    linkedInUrl: 'https://www.linkedin.com/in/marcusvance',
    dietaryTag: 'western',
    shareContactOnScan: true,
    isPlaceholder: true,
    interests: ['PostgreSQL', 'Single Sign-On', 'Agentic Engineering'],
  },
  {
    id: 'usr-dean-norman',
    email: 'dnorman@chadwickschool.org',
    fullName: 'Dean Norman',
    title: 'Director of Academic Technology & Systems',
    department: 'Information Technology',
    organization: 'Chadwick International',
    userType: 'internal_staff',
    role: 'event_organizer',
    avatarUrl: initialsAvatar('Dean Norman'),
    bio: 'Overseeing institutional technology strategy, cloud infrastructure, and data governance. Chair of the Summit Committee.',
    isDirectoryVisible: true,
    checkedIn: true,
    checkedInAt: 'Oct 15, 2027 07:30 AM',
    linkedInUrl: 'https://www.linkedin.com/in/deannorman',
    dietaryTag: 'western',
    shareContactOnScan: true,
    isPlaceholder: true,
    interests: ['Google Workspace Admin', 'Veracross SIS', 'Event Infrastructure'],
  },
  {
    id: 'usr-david-kim',
    email: 'dkim@chadwickschool.org',
    fullName: 'Prof. David Kim',
    title: 'Humanities & Academic Ethics Lead',
    department: 'Humanities',
    organization: 'Chadwick International',
    userType: 'internal_faculty',
    role: 'speaker',
    avatarUrl: initialsAvatar('Prof. David Kim'),
    bio: 'Sample speaker entry. Replace with the faculty member leading this session and the question their session opens.',
    isDirectoryVisible: true,
    checkedIn: false,
    linkedInUrl: 'https://www.linkedin.com/in/davidkim-humanities',
    dietaryTag: 'korean',
    shareContactOnScan: true,
    isPlaceholder: true,
    interests: ['Academic Integrity', 'AI Ethics', 'Philosophy'],
  },
  {
    id: 'usr-elena-rostova',
    email: 'elena.rostova@seoul-foreign.org',
    fullName: 'Elena Rostova',
    title: 'Director of Innovation & Learning',
    department: 'Curriculum & Pedagogy',
    organization: 'Seoul Foreign School',
    userType: 'external_guest',
    role: 'speaker',
    avatarUrl: initialsAvatar('Elena Rostova'),
    bio: 'Sample speaker entry. Replace with a colleague from a partner school, noting the practice they are bringing rather than a full CV.',
    isDirectoryVisible: true,
    checkedIn: true,
    checkedInAt: 'Oct 15, 2027 08:30 AM',
    linkedInUrl: 'https://www.linkedin.com/in/elenarostova',
    dietaryTag: 'vegan',
    shareContactOnScan: true,
    isPlaceholder: true,
    interests: ['Cross-School Collaboration', 'Design Thinking'],
  },
  {
    id: 'usr-jordan-hayes',
    email: 'jhayes@chadwickschool.org',
    fullName: 'Jordan Hayes',
    title: 'Campus Operations & AV Systems Manager',
    department: 'Campus Operations',
    organization: 'Chadwick International',
    userType: 'internal_staff',
    role: 'attendee',
    avatarUrl: initialsAvatar('Jordan Hayes'),
    bio: 'Managing acoustic calibration, venue streaming, and campus facility logistics.',
    isDirectoryVisible: false, // Opted out for privacy toggle demo!
    checkedIn: true,
    checkedInAt: 'Oct 15, 2027 07:45 AM',
    linkedInUrl: 'https://www.linkedin.com/in/jordanhayes-ops',
    dietaryTag: 'gluten_free',
    shareContactOnScan: false,
    isPlaceholder: true,
    interests: ['Audio Engineering', 'Live Streaming'],
  },
  {
    id: 'usr-meiling-zhou',
    email: 'mzhou@chadwickschool.org',
    fullName: 'Mei-Ling Zhou',
    title: 'Senior Mathematics Instructor',
    department: 'Mathematics',
    organization: 'Chadwick International',
    userType: 'internal_faculty',
    role: 'attendee',
    avatarUrl: initialsAvatar('Mei-Ling Zhou'),
    bio: 'Passionate about mathematical modeling, statistical intuition, and Python data science projects for students.',
    isDirectoryVisible: true,
    checkedIn: true,
    checkedInAt: 'Oct 15, 2027 08:22 AM',
    linkedInUrl: 'https://www.linkedin.com/in/meilingzhou',
    dietaryTag: 'vegetarian',
    shareContactOnScan: true,
    isPlaceholder: true,
    interests: ['Math Modeling', 'Data Visualization'],
  },
  {
    id: 'usr-chloe-bennett',
    email: 'cbennett27@chadwickschool.org',
    fullName: 'Chloe Bennett',
    title: 'Student Technology Ambassador',
    department: 'Upper School Council',
    organization: 'Chadwick International',
    userType: 'student',
    role: 'attendee',
    avatarUrl: initialsAvatar('Chloe Bennett'),
    bio: 'Senior student organizing high school robotics hackathons and student audio-visual support crew.',
    isDirectoryVisible: true,
    checkedIn: false,
    linkedInUrl: 'https://www.linkedin.com/in/chloebennett',
    dietaryTag: 'halal',
    shareContactOnScan: true,
    isPlaceholder: true,
    interests: ['Student Leadership', 'Hackathons'],
  },
  {
    id: 'usr-security-park',
    email: 'security@chadwickschool.org',
    fullName: 'Officer Jin-Ho Park',
    title: 'Campus Security Lead',
    department: 'Campus Safety & Operations',
    organization: 'Chadwick International',
    userType: 'internal_staff',
    role: 'front_desk',
    avatarUrl: initialsAvatar('Officer Jin-Ho Park'),
    bio: 'Campus safety lead responsible for venue access control and visitor verification during school events.',
    isDirectoryVisible: false,
    checkedIn: true,
    checkedInAt: 'Oct 15, 2027 06:45 AM',
    dietaryTag: 'korean',
    shareContactOnScan: false,
    isPlaceholder: true,
    interests: ['Campus Safety', 'Access Control'],
  },
];

export const INITIAL_SESSIONS: Session[] = [
  {
    id: 'sess-01-keynote',
    eventId: 'evt-mission-conference',
    roomId: 'room-theater',
    trackId: 'track-global',
    title: 'Opening Keynote: Human at the Centre in an Age of Intelligent Machines',
    description: 'Our mission asks us to develop global citizens with keen minds, exemplary character, self-knowledge and the ability to lead. What does each of those words demand of us now that machines can write, reason and persuade? A frank look at what technology should, and should not, be asked to carry.',
    day: 1,
    dateStr: 'Friday, Oct 15, 2027',
    startTime: '09:00 AM',
    endTime: '10:15 AM',
    startMinutes: 540,
    endMinutes: 615,
    maxAttendees: 600,
    reservedUserIds: ['usr-sarah-lin', 'usr-dean-norman', 'usr-david-kim', 'usr-elena-rostova', 'usr-meiling-zhou'],
    waitlistUserIds: [],
    speakerIds: ['usr-ted-hill', 'usr-dean-norman'],
    primarySponsorId: 'spon-mcgraw',
    supportingSponsorIds: ['spon-britannica', 'spon-faria', 'spon-upgrad'],
    slidesUrl: 'https://storage.googleapis.com/ci-conference-2026/keynote-architecture-v1.pdf',
    slidesName: 'Human-at-the-Centre-Keynote.pdf (14.2 MB)',
    isFeatured: true,
    tags: ['Keynote', 'Mission', 'AI & Humanity', 'Global Citizenship'],
  },
  {
    id: 'sess-02-ai-classroom',
    eventId: 'evt-mission-conference',
    roomId: 'room-b201',
    trackId: 'track-minds',
    title: 'Thinking With Machines, Not For Them: Prompt Literacy as Critical Inquiry',
    description: 'A hands-on workshop on using AI to sharpen student reasoning rather than replace it. Participants build prompts that demand evidence, surface assumptions and invite disagreement, then test them against real student work.',
    day: 1,
    dateStr: 'Friday, Oct 15, 2027',
    startTime: '10:45 AM',
    endTime: '12:00 PM',
    startMinutes: 645,
    endMinutes: 720,
    maxAttendees: 25,
    reservedUserIds: [
      'usr-sarah-lin', 'usr-david-kim', 'usr-meiling-zhou',
      'usr-20', 'usr-21', 'usr-22', 'usr-23', 'usr-24', 'usr-25', 'usr-26', 'usr-27', 'usr-28', 'usr-29',
      'usr-30', 'usr-31', 'usr-32', 'usr-33', 'usr-34', 'usr-35', 'usr-36', 'usr-37', 'usr-38', 'usr-39',
      'usr-40', 'usr-41', 'usr-42', 'usr-43', 'usr-44', 'usr-45' // 29 seats reserved! Almost full!
    ],
    waitlistUserIds: [],
    speakerIds: ['usr-sarah-lin'],
    primarySponsorId: 'spon-mcgraw',
    supportingSponsorIds: ['spon-britannica', 'spon-faria', 'spon-upgrad'],
    slidesUrl: 'https://storage.googleapis.com/ci-conference-2026/prompt-literacy-workshop.pdf',
    slidesName: 'Prompt-Literacy-Workshop.pdf (8.4 MB)',
    isFeatured: true,
    tags: ['Keen Minds', 'AI Literacy', 'Critical Thinking', 'Workshop'],
  },
  {
    id: 'sess-03-ethics-panel',
    eventId: 'evt-mission-conference',
    roomId: 'room-blackbox',
    trackId: 'track-character',
    title: 'Integrity When the Machine Can Write: Honour in an AI Era',
    description: 'Exemplary character is easy to name and hard to assess. A candid panel on what academic honesty means when the tools are this good, why detection alone fails, and how honour has to be taught rather than policed.',
    day: 1,
    dateStr: 'Friday, Oct 15, 2027',
    startTime: '10:45 AM',
    endTime: '12:00 PM',
    startMinutes: 645,
    endMinutes: 720,
    maxAttendees: 200,
    reservedUserIds: ['usr-dean-norman', 'usr-chloe-bennett'],
    waitlistUserIds: [],
    speakerIds: ['usr-david-kim', 'usr-chloe-bennett'],
    primarySponsorId: 'spon-incheon',
    supportingSponsorIds: ['spon-edevents', 'spon-scalaedu'],
    slidesUrl: 'https://storage.googleapis.com/ci-conference-2026/academic-integrity-panel.pdf',
    slidesName: 'Integrity-in-an-AI-Era.pdf (5.1 MB)',
    tags: ['Exemplary Character', 'Academic Integrity', 'Panel', 'Honour'],
  },
  {
    id: 'sess-04-lunch-collab',
    eventId: 'evt-mission-conference',
    roomId: 'room-theater',
    trackId: 'track-global',
    title: 'Community Luncheon & Cross-Divisional Conversations',
    description: 'Lunch with intent. Tables are grouped by mission strand rather than by division, so Lower School and Upper School colleagues end up discussing the same question from opposite ends of a student journey.',
    day: 1,
    dateStr: 'Friday, Oct 15, 2027',
    startTime: '12:15 PM',
    endTime: '01:30 PM',
    startMinutes: 735,
    endMinutes: 810,
    maxAttendees: 600,
    reservedUserIds: ['usr-sarah-lin', 'usr-marcus-vance', 'usr-dean-norman', 'usr-elena-rostova'],
    waitlistUserIds: [],
    speakerIds: ['usr-dean-norman'],
    primarySponsorId: 'spon-incheon',
    supportingSponsorIds: ['spon-edevents', 'spon-scalaedu'],
    tags: ['Community', 'Networking', 'Cross-Divisional'],
  },
  {
    id: 'sess-05-robotics-maker',
    eventId: 'evt-mission-conference',
    roomId: 'room-cihub',
    trackId: 'track-minds',
    title: 'Inquiry-Based Robotics: Building Cross-Disciplinary Challenges',
    description: 'How an open-ended robotics challenge develops keen minds across subjects — the engineering is the visible part, but the reasoning, iteration and argument are the point. Includes a challenge design template to take back to your division.',
    day: 1,
    dateStr: 'Friday, Oct 15, 2027',
    startTime: '01:45 PM',
    endTime: '03:00 PM',
    startMinutes: 825,
    endMinutes: 900,
    maxAttendees: 40,
    reservedUserIds: ['usr-sarah-lin', 'usr-meiling-zhou'],
    waitlistUserIds: [],
    speakerIds: ['usr-elena-rostova'],
    primarySponsorId: 'spon-rustic',
    supportingSponsorIds: ['spon-creverse', 'spon-poin2lab'],
    slidesUrl: 'https://storage.googleapis.com/ci-conference-2026/robotics-stem-curriculum.pdf',
    slidesName: 'Inquiry-Robotics-Challenges.pdf (11.7 MB)',
    tags: ['Keen Minds', 'Inquiry', 'STEM', 'Cross-Disciplinary'],
  },
  {
    id: 'sess-06-veracross-sync',
    eventId: 'evt-mission-conference',
    roomId: 'room-b202',
    trackId: 'track-lead',
    title: 'Systems That Serve People: Designing School Operations Around Learning',
    description: 'A working session on the unglamorous half of leadership — timetables, rooms, rosters and the systems beneath them. How we choose and build tools so they absorb administrative friction rather than adding it, leaving teachers free to teach.',
    day: 1,
    dateStr: 'Friday, Oct 15, 2027',
    startTime: '01:45 PM',
    endTime: '03:00 PM',
    startMinutes: 825,
    endMinutes: 900,
    maxAttendees: 25,
    // Filled completely to demonstrate capacity lock & waitlist!
    reservedUserIds: Array.from({ length: 25 }, (_, i) => `usr-filled-${i}`),
    waitlistUserIds: ['usr-david-kim'],
    speakerIds: ['usr-marcus-vance', 'usr-dean-norman'],
    slidesUrl: 'https://storage.googleapis.com/ci-conference-2026/veracross-google-sync.pdf',
    slidesName: 'Systems-That-Serve-People.pdf (6.3 MB)',
    tags: ['Leadership', 'Operations', 'Systems Design', 'Human-Centred Tech'],
  },
  {
    id: 'sess-07-math-dataviz',
    eventId: 'evt-mission-conference',
    roomId: 'room-b201',
    trackId: 'track-global',
    title: 'Reading the World in Data: Climate, Migration and Global Citizenship',
    description: 'Global citizenship needs more than sympathy — it needs the ability to read the evidence. Working with real climate and migration datasets, this session builds the statistical judgement students need to hold an informed position, and to notice when a chart is arguing with them.',
    day: 2,
    dateStr: 'Saturday, Oct 16, 2027',
    startTime: '09:00 AM',
    endTime: '10:15 AM',
    startMinutes: 540,
    endMinutes: 615,
    maxAttendees: 25,
    reservedUserIds: ['usr-sarah-lin'],
    waitlistUserIds: [],
    speakerIds: ['usr-meiling-zhou'],
    primarySponsorId: 'spon-rustic',
    supportingSponsorIds: ['spon-creverse', 'spon-poin2lab'],
    slidesUrl: 'https://storage.googleapis.com/ci-conference-2026/math-dataviz.pdf',
    slidesName: 'Reading-the-World-in-Data.pdf (9.8 MB)',
    tags: ['Global Citizenship', 'Data Literacy', 'Climate', 'Evidence'],
  },
  {
    id: 'sess-08-wellbeing-digital',
    eventId: 'evt-mission-conference',
    roomId: 'room-blackbox',
    trackId: 'track-self',
    title: 'Attention, Balance and Self-Knowledge on a Connected Campus',
    description: 'Self-knowledge is the strand most easily crowded out by a device. Practical approaches to helping students notice their own attention, name what a tool is doing to it, and choose deliberately — drawing on what we have learned from our own 1:1 programme.',
    day: 2,
    dateStr: 'Saturday, Oct 16, 2027',
    startTime: '09:00 AM',
    endTime: '10:15 AM',
    startMinutes: 540,
    endMinutes: 615,
    maxAttendees: 200,
    reservedUserIds: ['usr-david-kim', 'usr-elena-rostova'],
    waitlistUserIds: [],
    speakerIds: ['usr-david-kim'],
    tags: ['Self-Knowledge', 'Wellbeing', 'Attention', 'Digital Balance'],
  },
  {
    id: 'sess-09-closing-raffle',
    eventId: 'evt-mission-conference',
    roomId: 'room-theater',
    trackId: 'track-lead',
    title: 'Closing Plenary: The Ability to Lead, and What We Do on Monday',
    description: 'The conference closes where the mission does — with the ability to lead. Strand leads report what emerged, we name the commitments worth carrying into the year, and the community draw sends everyone home. Leadership here means whoever chooses to act first.',
    day: 2,
    dateStr: 'Saturday, Oct 16, 2027',
    startTime: '03:30 PM',
    endTime: '04:45 PM',
    startMinutes: 930,
    endMinutes: 1005,
    maxAttendees: 600,
    reservedUserIds: ['usr-sarah-lin', 'usr-marcus-vance', 'usr-dean-norman', 'usr-david-kim', 'usr-elena-rostova', 'usr-meiling-zhou'],
    waitlistUserIds: [],
    speakerIds: ['usr-dean-norman', 'usr-marcus-vance'],
    primarySponsorId: 'spon-mcgraw',
    supportingSponsorIds: ['spon-britannica', 'spon-faria', 'spon-upgrad'],
    isFeatured: true,
    tags: ['Leadership', 'Plenary', 'Commitments', 'Closing'],
  },
  {
    id: 'sess-10-wellness-belonging',
    eventId: 'evt-mission-conference',
    roomId: 'room-blackbox',
    trackId: 'track-self',
    title: 'Wellness and Belonging: The Conditions Learning Needs',
    description: 'Keen minds do not grow in anxious rooms. An honest session on the conditions that have to hold before any of the rest of the mission is possible: belonging, psychological safety, workload, and the quiet signals that tell a student whether this place is theirs.',
    day: 1,
    dateStr: 'Friday, Oct 15, 2027',
    startTime: '01:45 PM',
    endTime: '03:00 PM',
    startMinutes: 825,
    endMinutes: 900,
    maxAttendees: 200,
    reservedUserIds: ['usr-meiling-zhou', 'usr-chloe-bennett'],
    waitlistUserIds: [],
    speakerIds: ['usr-elena-rostova'],
    tags: ['Self-Knowledge', 'Wellness', 'Belonging', 'Inclusion'],
  },
  {
    id: 'sess-11-chadwick-way',
    eventId: 'evt-mission-conference',
    roomId: 'room-theater',
    trackId: 'track-lead',
    title: 'The Chadwick Way: Framing Emerging Technology Against a Mission',
    description: 'Most schools decide about technology tool by tool. This session makes the case for deciding against a mission instead, offering a working framework for asking what a new capability does to global citizenship, keen minds, character, self-knowledge and leadership before asking what it costs.',
    day: 2,
    dateStr: 'Saturday, Oct 16, 2027',
    startTime: '10:45 AM',
    endTime: '12:00 PM',
    startMinutes: 645,
    endMinutes: 720,
    maxAttendees: 600,
    reservedUserIds: ['usr-sarah-lin', 'usr-dean-norman', 'usr-david-kim'],
    waitlistUserIds: [],
    speakerIds: ['usr-dean-norman', 'usr-david-kim'],
    tags: ['Leadership', 'The Chadwick Way', 'AI Strategy', 'Decision Framework'],
  },
  {
    id: 'sess-12-belonging-design',
    eventId: 'evt-mission-conference',
    roomId: 'room-cihub',
    trackId: 'track-global',
    title: 'Belonging by Design: Inclusive Practice Across Four Divisions',
    description: 'What inclusion looks like when it is structural rather than aspirational. Colleagues from Early Years through Upper School trace one practice across a whole student journey, and the visiting cohort is invited to push back with what works elsewhere.',
    day: 2,
    dateStr: 'Saturday, Oct 16, 2027',
    startTime: '10:45 AM',
    endTime: '12:00 PM',
    startMinutes: 645,
    endMinutes: 720,
    maxAttendees: 40,
    reservedUserIds: ['usr-elena-rostova', 'usr-meiling-zhou'],
    waitlistUserIds: [],
    speakerIds: ['usr-meiling-zhou', 'usr-sarah-lin'],
    tags: ['Global Citizenship', 'Inclusivity', 'Belonging', 'Cross-Divisional'],
  },
  {
    id: 'sess-13-inside-chadwick',
    eventId: 'evt-mission-conference',
    roomId: 'room-b203',
    trackId: 'track-self',
    title: 'Inside Chadwick: Campus Walkthrough and Open Conversation',
    description: 'For visiting colleagues curious about how this actually works day to day. Walk the campus with the people who teach in it, sit in on a space mid-use, and ask the unguarded questions about the school, the community, and what it is like to build a career here.',
    day: 2,
    dateStr: 'Saturday, Oct 16, 2027',
    startTime: '01:45 PM',
    endTime: '03:00 PM',
    startMinutes: 825,
    endMinutes: 900,
    maxAttendees: 25,
    reservedUserIds: ['usr-marcus-vance'],
    waitlistUserIds: [],
    speakerIds: ['usr-dean-norman', 'usr-chloe-bennett'],
    tags: ['Discover Chadwick', 'Community', 'Campus Visit', 'Careers'],
  },
];

export const INITIAL_COMMUNITY_TOPICS: CommunityTopic[] = [
  {
    id: 'comm-01',
    eventId: 'evt-mission-conference',
    authorId: 'usr-sarah-lin',
    authorName: 'Dr. Sarah Lin',
    authorEmail: 'slin@chadwickschool.org',
    authorAvatar: initialsAvatar('Dr. Sarah Lin'),
    authorDepartment: 'STEM & Computer Science',
    authorType: 'internal_faculty',
    title: 'Lunch Meetup: Keen Minds strand — where is AI actually helping?',
    content: 'Let\'s gather during Friday lunch (around 12:30 PM) at the Outdoor Courtyard shaded picnic tables! We can discuss what prompt frameworks you\'re testing with 9th-12th graders and share curricula.',
    category: 'Informal Meetups',
    location: 'Campus Outdoor Courtyard, Table 4',
    meetupTime: 'Friday, Oct 15 @ 12:30 PM',
    rsvpUserIds: ['usr-sarah-lin', 'usr-meiling-zhou', 'usr-elena-rostova'],
    createdAt: '2 hours ago',
    likesCount: 14,
    replies: [
      {
        id: 'rep-1',
        authorName: 'Mei-Ling Zhou',
        authorAvatar: initialsAvatar('Mei-Ling Zhou'),
        authorDepartment: 'Mathematics',
        content: 'Count me in! I would love to see how you integrate basic Python scripts without intimidating algebra students.',
        createdAt: '1 hour ago',
      },
      {
        id: 'rep-2',
        authorName: 'Elena Rostova',
        authorAvatar: initialsAvatar('Elena Rostova'),
        authorDepartment: 'Seoul Foreign School',
        content: 'I\'ll bring our 3D-printed chassis examples to share with the group!',
        createdAt: '35 mins ago',
      },
    ],
  },
  {
    id: 'comm-02',
    eventId: 'evt-mission-conference',
    authorId: 'usr-dean-norman',
    authorName: 'Dean Norman',
    authorEmail: 'dnorman@chadwickschool.org',
    authorAvatar: initialsAvatar('Dean Norman'),
    authorDepartment: 'Information Technology',
    authorType: 'internal_staff',
    title: 'Organiser Desk: Campus Wi-Fi & Slide Upload Instructions',
    content: 'Welcome everyone! For all speakers, please make sure your final presentation slides are submitted via the session drawer or directly to AV desk by 8:30 AM. Campus network is "Summit-Guest" with credentials in your badge lanyard.',
    category: 'Ask Organizers',
    location: 'Auditorium Tech Booth',
    rsvpUserIds: ['usr-dean-norman'],
    createdAt: '5 hours ago',
    likesCount: 22,
    replies: [
      {
        id: 'rep-3',
        authorName: 'Prof. David Kim',
        authorAvatar: initialsAvatar('Prof. David Kim'),
        authorDepartment: 'Humanities',
        content: 'Slides uploaded for the ethics panel! Thanks to the tech crew for the swift confirmation.',
        createdAt: '4 hours ago',
      },
    ],
  },
  {
    id: 'comm-03',
    eventId: 'evt-mission-conference',
    authorId: 'usr-elena-rostova',
    authorName: 'Elena Rostova',
    authorEmail: 'elena.rostova@seoul-foreign.org',
    authorAvatar: initialsAvatar('Elena Rostova'),
    authorDepartment: 'Seoul Foreign School',
    authorType: 'external_guest',
    title: 'Carpool / Shared Taxi from Songdo Central Park to Campus',
    content: 'Is anyone staying near Songdo Central Park Hotel on Saturday morning? Planning to catch a shared taxi around 8:00 AM to arrive in time for the 9:00 AM sessions.',
    category: 'Carpool & Dinners',
    location: 'Songdo Central Park Hotel Lobby',
    meetupTime: 'Saturday, Oct 17 @ 08:00 AM',
    rsvpUserIds: ['usr-elena-rostova', 'usr-marcus-vance'],
    createdAt: 'Yesterday',
    likesCount: 8,
    replies: [
      {
        id: 'rep-4',
        authorName: 'Marcus Vance',
        authorAvatar: initialsAvatar('Marcus Vance'),
        authorDepartment: 'Vance Systems',
        content: 'I am right at the hotel! Let\'s coordinate in the lobby at 8:00 AM sharp.',
        createdAt: 'Yesterday',
      },
    ],
  },
];

export const INITIAL_ANNOUNCEMENTS: BroadcastAnnouncement[] = [
  {
    id: 'ann-1',
    title: 'Room Update',
    message: 'Friday Afternoon Panel on Academic Integrity relocated to Black Box to accommodate higher attendee interest.',
    priority: 'normal',
    timestamp: '15 mins ago',
    active: true,
  },
];

export const SAMPLE_CSV_TEMPLATE = `Title,Description,Date,StartTime,EndTime,Track,TrackColor,Room,RoomCapacity,SpeakerName,SpeakerEmail,SpeakerTitle
"Argument in the Age of Autocomplete","Teaching students to defend a position a model could have written for them","2027-10-15","11:00 AM","12:15 PM","Keen Minds","#2A6791","B-201",30,"Dr. Sarah Lin","slin@chadwickschool.org","CS Department Chair"
"Quiet Leadership: Growing Colleagues, Not Followers","How middle leaders build capacity without adding load","2027-10-15","01:45 PM","03:00 PM","Leadership","#B04318","Black Box",55,"Prof. David Kim","dkim@chadwickschool.org","Ethics Lead"
"Service Beyond the Service Trip","Designing global citizenship that outlasts a week abroad","2027-10-16","10:30 AM","11:45 AM","Global Citizenship","#002B54","Collaboration and Innovation Hub",40,"Elena Rostova","elena@rostova-learning.org","Learning Designer"`;


/* ============================================================
   Dining services. Options carry a DietaryTag so a selection can be
   surfaced on the badge for catering staff without them having to look
   anyone up.
   ============================================================ */
export const INITIAL_MEAL_SERVICES: MealService[] = [
  {
    id: 'meal-d1-coffee',
    name: 'Morning Coffee & Pastries',
    type: 'snack',
    day: 1,
    dateStr: 'Friday, Oct 15, 2027',
    startTime: '08:00 AM',
    endTime: '09:00 AM',
    startMinutes: 480,
    endMinutes: 540,
    location: 'Main Theater — Foyer',
    options: [
      { id: 'opt-d1c-western', label: 'Western Pastries', dietary: 'western', description: 'Croissants, danishes, muffins, brewed coffee and tea.' },
      { id: 'opt-d1c-korean', label: 'Korean Breakfast Bites', dietary: 'korean', description: 'Gyeranppang, rice cakes, barley tea.' },
      { id: 'opt-d1c-gf', label: 'Gluten-Free Selection', dietary: 'gluten_free', description: 'Certified gluten-free baked goods, prepared on a separate line.', maxServings: 30 },
    ],
    selections: {},
  },
  {
    id: 'meal-d1-lunch',
    name: 'Day 1 Lunch',
    type: 'lunch',
    day: 1,
    dateStr: 'Friday, Oct 15, 2027',
    startTime: '12:15 PM',
    endTime: '01:30 PM',
    startMinutes: 735,
    endMinutes: 810,
    location: 'Dining Commons — Main Hall',
    options: [
      { id: 'opt-d1l-western', label: 'Western Hot Buffet', dietary: 'western', description: 'Roast chicken, herb potatoes, seasonal vegetables, garden salad.', maxServings: 180 },
      { id: 'opt-d1l-korean', label: 'Korean Set Menu', dietary: 'korean', description: 'Bibimbap with assorted banchan and doenjang jjigae.', maxServings: 150 },
      { id: 'opt-d1l-veg', label: 'Vegetarian Plate', dietary: 'vegetarian', description: 'Roasted vegetable orzo, halloumi skewers, lentil salad.', maxServings: 90 },
      { id: 'opt-d1l-vegan', label: 'Vegan Plate', dietary: 'vegan', description: 'Chickpea tagine, herbed couscous, grilled seasonal vegetables.', maxServings: 45 },
      { id: 'opt-d1l-halal', label: 'Halal Set Menu', dietary: 'halal', description: 'Halal-certified braised beef, saffron rice, cucumber salad.', maxServings: 40 },
      { id: 'opt-d1l-gf', label: 'Gluten-Free Plate', dietary: 'gluten_free', description: 'Grilled salmon, quinoa pilaf, roasted root vegetables.', maxServings: 30 },
    ],
    selections: {
      'usr-sarah-lin': 'opt-d1l-veg',
      'usr-dean-norman': 'opt-d1l-western',
      'usr-david-kim': 'opt-d1l-korean',
      'usr-elena-rostova': 'opt-d1l-vegan',
      'usr-chloe-bennett': 'opt-d1l-halal',
    },
  },
  {
    id: 'meal-d1-snack',
    name: 'Afternoon Refreshments',
    type: 'snack',
    day: 1,
    dateStr: 'Friday, Oct 15, 2027',
    startTime: '03:00 PM',
    endTime: '03:30 PM',
    startMinutes: 900,
    endMinutes: 930,
    location: 'Library — Innovation Hub Atrium',
    options: [
      { id: 'opt-d1s-standard', label: 'Assorted Snacks', dietary: 'western', description: 'Fruit, biscuits, iced coffee and infused water.' },
      { id: 'opt-d1s-nutfree', label: 'Nut-Free Selection', dietary: 'nut_free', description: 'Prepared and served on a dedicated nut-free station.', maxServings: 40 },
    ],
    selections: {},
  },
  {
    id: 'meal-d2-lunch',
    name: 'Day 2 Lunch',
    type: 'lunch',
    day: 2,
    dateStr: 'Saturday, Oct 16, 2027',
    startTime: '12:00 PM',
    endTime: '01:15 PM',
    startMinutes: 720,
    endMinutes: 795,
    location: 'Dining Commons — Main Hall',
    options: [
      { id: 'opt-d2l-western', label: 'Western Hot Buffet', dietary: 'western', description: 'Baked pasta, garlic focaccia, Caesar salad.', maxServings: 180 },
      { id: 'opt-d2l-korean', label: 'Korean Set Menu', dietary: 'korean', description: 'Bulgogi, steamed rice, kimchi and seasonal banchan.', maxServings: 150 },
      { id: 'opt-d2l-veg', label: 'Vegetarian Plate', dietary: 'vegetarian', description: 'Mushroom risotto, rocket and pear salad.', maxServings: 90 },
      { id: 'opt-d2l-vegan', label: 'Vegan Plate', dietary: 'vegan', description: 'Tofu bibimbap with gochujang, no egg.', maxServings: 45 },
      { id: 'opt-d2l-halal', label: 'Halal Set Menu', dietary: 'halal', description: 'Halal-certified lamb kofta, flatbread, tabbouleh.', maxServings: 40 },
    ],
    selections: { 'usr-sarah-lin': 'opt-d2l-veg' },
  },
  {
    id: 'meal-d2-reception',
    name: 'Closing Reception',
    type: 'reception',
    day: 2,
    dateStr: 'Saturday, Oct 16, 2027',
    startTime: '04:30 PM',
    endTime: '06:00 PM',
    startMinutes: 990,
    endMinutes: 1080,
    location: 'Performing Arts Center — Terrace',
    options: [
      { id: 'opt-d2r-canape', label: 'Canapé Reception', dietary: 'western', description: 'Passed canapés, sparkling and soft drinks.' },
      { id: 'opt-d2r-veg', label: 'Vegetarian Canapés', dietary: 'vegetarian', description: 'Dedicated vegetarian passing trays.', maxServings: 60 },
      { id: 'opt-d2r-halal', label: 'Halal Canapés', dietary: 'halal', description: 'Halal-certified passing trays.', maxServings: 35 },
    ],
    selections: {},
  },
];

/* Attendance is written by door scans. Seeded with a couple of records so the
   organizer audit view has something to reconcile. */
export const INITIAL_ATTENDANCE: AttendanceRecord[] = [
  {
    id: 'att-001',
    userId: 'usr-sarah-lin',
    sessionId: 'sess-01-keynote',
    scannedAt: 'Oct 15, 2027 08:58 AM',
    location: 'Main Theater (Level 1)',
    status: 'verified',
    scannedBy: 'usr-dean-norman',
  },
  {
    id: 'att-002',
    userId: 'usr-chloe-bennett',
    sessionId: 'sess-01-keynote',
    scannedAt: 'Oct 15, 2027 09:04 AM',
    location: 'Main Theater (Level 1)',
    status: 'walk_in',
    scannedBy: 'usr-dean-norman',
  },
];

export const INITIAL_MESSAGES: DirectMessage[] = [
  {
    id: 'msg-001',
    fromUserId: 'usr-david-kim',
    toUserId: 'usr-sarah-lin',
    content: 'Sarah — enjoyed your framing on student AI agency. Could we compare notes on the Humanities rubric before your Friday session?',
    createdAt: '2 hours ago',
    read: false,
  },
  {
    id: 'msg-002',
    fromUserId: 'usr-elena-rostova',
    toUserId: 'usr-sarah-lin',
    content: 'Hello! Following up from the workshop — happy to share our district rollout deck if useful.',
    createdAt: '5 hours ago',
    read: true,
  },
];


/* ============================================================
   Public event page content. Swap this object to re-skin the public site
   for a different event without touching the portal.
   ============================================================ */
export const EVENT_CONFIG: EventConfig = {
  id: 'evt-mission-conference',
  slug: 'chadwick-connects-2027',
  registrationOpensAt: '2027-03-01',
  category: 'Conference',
  isFeatured: true,
  hasPortal: true,
  summary:
    'Our flagship gathering. The Chadwick Schools community together with educators and experts from around the world, across the five mission strands, on the conviction that relationships drive our mission.',
  name: 'Chadwick Connects',
  shortName: 'Chadwick Connects',
  tagline: 'Relationships Drive Our Mission',
  description:
    'Chadwick Connects brings the Chadwick Schools community together with educators and experts from around the world for two days built on a single conviction: relationships drive our mission. The programme runs across the five strands of that mission — global citizens with keen minds, exemplary character, self-knowledge and the ability to lead — and asks what each demands of us as our tools begin to think.',
  startDate: '2027-10-15',
  endDate: '2027-10-16',
  dateLabel: 'Friday 15 - Saturday 16 October 2027',
  venueName: 'Chadwick International, Songdo',
  venueAddress: '45 Art center-daero 97beon-gil, Yeonsu-gu, Incheon, Republic of Korea',
  heroImageUrl:
    'https://images.unsplash.com/photo-1523050854058-8df90110c9f1?w=1600&auto=format&fit=crop&q=80',
  registrationOpen: true,
  isTemplate: true,
  registrationNote:
    'Chadwick faculty, staff and students sign in with their school Google account. Visiting educators use the access code sent with their invitation.',
  highlights: [
    { label: 'Mission strands', value: '5' },
    { label: 'Sessions', value: '40+' },
    { label: 'Countries represented', value: '18' },
    { label: 'Days', value: '2' },
  ],
  about: [
    {
      heading: 'Relationships drive our mission',
      body: 'Chadwick has always held that who a student learns alongside matters as much as what they learn. Chadwick Connects applies that to the adults: two campuses, visiting colleagues from around the world, and a programme built for conversation rather than broadcast. Every session sits under one of the five mission strands, so the two days add up to something about who we are helping students become.',
    },
    {
      heading: 'Future-forward, framed the Chadwick Way',
      body: 'Artificial intelligence runs through all five strands rather than sitting in a track of its own, because that is how it arrives in a school. Alongside it sit the things that decide whether any of it lands — wellness, belonging and inclusion — and the recurring question is not what a technology can do, but which parts of our mission it should be nowhere near.',
    },
    {
      heading: 'Two campuses, many schools, one room',
      body: 'The Chadwick Schools community convenes alongside educators and experts from international and independent schools worldwide. The schedule protects the talking that happens between sessions, and the visiting cohort is treated as a source of practice rather than an audience for ours.',
    },
  ],
  discover: {
    heading: 'Come for the conference, meet the community',
    intro:
      'The conference is also, unapologetically, an invitation. Two days on campus is the most honest way to understand a school — how it sounds between lessons, how colleagues talk to each other, whether the mission on the wall matches the one in the corridors.',
    points: [
      {
        title: 'See the school at work',
        body: 'Sessions run in real teaching spaces during a normal week. Walk the campus with the people who teach in it and sit in on a space mid-use, rather than touring an empty building.',
      },
      {
        title: 'Meet the people, not the pitch',
        body: 'Community lunches are grouped by mission strand rather than by role or division, so conversations start with the work. The colleague directory stays open after the event for anyone you want to keep talking to.',
      },
      {
        title: 'Explore joining us',
        body: 'If the way we think about teaching resonates, we would rather you discovered that over two days than from a job posting. Open conversations about working and learning at Chadwick run throughout, with no application and no obligation.',
      },
    ],
    closing:
      'Mission alignment is not something a CV shows. It is something you notice in a room.',
  },
  faqs: [
    {
      question: 'How is the programme organised?',
      answer: 'Around the five strands of our mission - Global Citizenship, Keen Minds, Exemplary Character, Self-Knowledge and Leadership. You can follow one strand across both days or move between them; the agenda filters either way.',
    },
    {
      question: 'Who can attend?',
      answer: 'Educators, school leaders and support staff from international and independent schools worldwide, alongside Chadwick International faculty, staff and student ambassadors as the host community.',
    },
    {
      question: 'I am curious about working at Chadwick. Is this the right event?',
      answer: 'Yes, and deliberately so. Sessions run in real teaching spaces, community lunches mix visitors with colleagues, and the Inside Chadwick walkthrough is built for exactly these conversations. There is no application process attached and no obligation - come and see whether the way we work fits how you want to teach.',
    },
    {
      question: 'How do I sign in?',
      answer: 'Chadwick community members use their @chadwickschool.org Google account. Visiting educators receive an access code by email once registration is confirmed, and use it with their invitation email address.',
    },
    {
      question: 'Are meals provided?',
      answer: 'Yes. Lunch and refreshments are included both days. Once signed in you can pre-select your meal - including vegetarian, vegan, halal, gluten-free and Korean options - so catering can plan accurately.',
    },
    {
      question: 'Do I need to reserve individual sessions?',
      answer: 'Rooms have fixed capacity and popular sessions do fill. Reserve your seat in the portal ahead of time; if a session is full you can join the waitlist and are promoted automatically when a seat frees up.',
    },
    {
      question: 'Is there a record of what I attended?',
      answer: 'Your badge is scanned at each session door, producing a verified attendance record you can use for professional growth and reporting.',
    },
  ],
};

/* ============================================================
   The events catalogue. CI Connects runs the whole year, not one conference,
   so the hub reads from this list and each entry renders its own landing page
   at /?event=<slug>. Only the flagship carries a full portal today; the rest
   are landing pages, which is the honest state of the platform.
   ============================================================ */
export const EVENTS: EventConfig[] = [
  EVENT_CONFIG,

  {
    id: 'evt-open-house-2026',
    slug: 'admissions-open-house-nov-2026',
    registrationClosesAt: '2026-11-10',
    category: 'Admissions',
    name: 'Autumn Admissions Open House',
    shortName: 'Open House',
    tagline: 'See the school on an ordinary Tuesday',
    summary:
      'Prospective families walk the campus during a normal teaching day, sit in on lessons, and meet the faculty their children would actually learn from.',
    startDate: '2026-11-14',
    endDate: '2026-11-14',
    dateLabel: 'Saturday 14 November 2026',
    venueName: 'Chadwick International, Songdo',
    heroImageUrl: 'https://images.unsplash.com/photo-1580582932707-520aed937b7b?w=1200&auto=format&fit=crop&q=80',
    registrationOpen: true,
    isTemplate: true,
    highlights: [
      { label: 'Divisions', value: '4' },
      { label: 'Open lessons', value: '20+' },
      { label: 'Hours', value: '4' },
    ],
  },

  {
    id: 'evt-student-forum-2027',
    slug: 'global-citizenship-student-forum-2027',
    registrationOpensAt: '2026-12-01',
    category: 'Student',
    name: 'Global Citizenship Student Forum',
    shortName: 'Student Forum',
    tagline: 'Students setting the agenda',
    summary:
      'Upper School delegations from across the region convene on a single question of consequence. Students chair every session; adults are permitted to listen.',
    startDate: '2027-02-20',
    endDate: '2027-02-21',
    dateLabel: 'Saturday 20 - Sunday 21 February 2027',
    venueName: 'Chadwick International, Songdo',
    heroImageUrl: 'https://images.unsplash.com/photo-1523240795612-9a054b0db644?w=1200&auto=format&fit=crop&q=80',
    registrationOpen: false,
    isTemplate: true,
    highlights: [
      { label: 'Delegations', value: '14' },
      { label: 'Student chairs', value: '100%' },
      { label: 'Days', value: '2' },
    ],
  },

  {
    id: 'evt-ai-practice-2027',
    slug: 'ai-in-practice-series-2027',
    category: 'Workshop',
    name: 'AI in Practice: Twilight Workshop Series',
    shortName: 'AI in Practice',
    tagline: 'Six Thursdays, one department at a time',
    summary:
      'A working series rather than a keynote. Departments bring a real unit and leave having rebuilt one assessment around what the tools have actually changed.',
    startDate: '2027-01-14',
    endDate: '2027-02-25',
    dateLabel: 'Six Thursdays, January - February 2027',
    venueName: 'Collaboration and Innovation Hub, Chadwick International',
    heroImageUrl: 'https://images.unsplash.com/photo-1516321318423-f06f85e504b3?w=1200&auto=format&fit=crop&q=80',
    registrationOpen: true,
    isTemplate: true,
    highlights: [
      { label: 'Sessions', value: '6' },
      { label: 'Departments', value: '9' },
      { label: 'Format', value: 'Hands-on' },
    ],
  },

  {
    id: 'evt-onboarding-2026',
    slug: 'educator-onboarding-institute-2026',
    category: 'Community',
    name: 'New Educator Onboarding Institute',
    shortName: 'Onboarding Institute',
    tagline: 'Three days before the students arrive',
    summary:
      'Incoming faculty met the mission, the campus and each other before the first bell — and the cohort that went through it is still meeting monthly.',
    startDate: '2026-08-10',
    endDate: '2026-08-12',
    dateLabel: '10 - 12 August 2026',
    venueName: 'Chadwick International, Songdo',
    heroImageUrl: 'https://images.unsplash.com/photo-1524178232363-1fb2b075b655?w=1200&auto=format&fit=crop&q=80',
    registrationOpen: false,
    isTemplate: true,
    outcomes: [
      { label: 'New colleagues', value: '31' },
      { label: 'Countries of origin', value: '11' },
      { label: 'Still meeting monthly', value: 'Yes' },
    ],
    recap: {
      summary:
        'Three days before term for educators new to Chadwick, ending with each participant teaching a fifteen-minute segment to their new colleagues.',
      takeaways: [
        'The mission strands are learnable in a week; the culture around them takes a term. Pair people accordingly.',
        'New staff wanted fewer sessions and more time in the rooms they would actually teach in.',
      ],
    },
  },

  {
    id: 'evt-wellbeing-summit-2026',
    slug: 'wellbeing-belonging-summit-2026',
    category: 'Symposium',
    name: 'Wellbeing & Belonging Summit',
    shortName: 'Wellbeing Summit',
    tagline: 'The conditions learning needs',
    summary:
      'Counsellors, tutors and division heads spent a day on the unglamorous groundwork: workload, belonging, and the signals a school sends without meaning to.',
    startDate: '2026-05-08',
    endDate: '2026-05-08',
    dateLabel: 'Friday 8 May 2026',
    venueName: 'Humanities Wing, Chadwick International',
    heroImageUrl: 'https://images.unsplash.com/photo-1544027993-37dbfe43562a?w=1200&auto=format&fit=crop&q=80',
    registrationOpen: false,
    isTemplate: true,
    outcomes: [
      { label: 'Attendees', value: '124' },
      { label: 'Schools', value: '9' },
      { label: 'Policies revised', value: '3' },
    ],
    recap: {
      summary:
        'A summit that spent as long on belonging as on wellbeing, on the argument that the first is usually the cause of the second.',
      takeaways: [
        'Students named arrival and transition points — not lessons — as where belonging is won or lost.',
        'Advisory time works when it is protected and predictable, and not otherwise.',
      ],
      materials: [
        { label: 'Summit summary and next steps', url: 'https://docs.google.com/document/d/SAMPLE-REPLACE-ME/edit' },
      ],
    },
  },

  {
    id: 'evt-future-learning-2026',
    slug: 'future-of-learning-symposium-2026',
    category: 'Symposium',
    name: 'Future of Learning Symposium',
    shortName: 'Future of Learning',
    tagline: 'The conversation that started this one',
    summary:
      'The symposium whose closing question - what is a school for, once the answers are free? - became the premise of the Mission Conference.',
    startDate: '2026-03-06',
    endDate: '2026-03-07',
    dateLabel: '6 - 7 March 2026',
    venueName: 'Main Theater, Chadwick International',
    heroImageUrl: 'https://images.unsplash.com/photo-1531482615713-2afd69097998?w=1200&auto=format&fit=crop&q=80',
    registrationOpen: false,
    isTemplate: true,
    outcomes: [
      { label: 'Attendees', value: '210' },
      { label: 'Sessions', value: '28' },
      { label: 'Countries', value: '12' },
    ],
    recap: {
      summary:
        'Two days that ended on a question nobody could answer tidily: what is a school for, once the answers are free? It became the premise of Chadwick Connects, and the closing plenary is worth watching in full.',
      recordingUrl: 'https://www.youtube.com/watch?v=jNQXAC9IVRw', // SAMPLE — replace
      recordingLabel: 'Watch the closing plenary',
      highlights: [
        { label: 'Attendees', value: '210' },
        { label: 'Sessions', value: '28' },
        { label: 'Countries', value: '12' },
        { label: 'Satisfaction', value: '4.6' },
      ],
      takeaways: [
        'The question is not what to teach when answers are free, but what to assess. Every school in the room had changed its teaching faster than its assessment.',
        'Departments that redesigned one unit together made more progress in a term than those that wrote a policy.',
        'Students asked to be taught how to argue with a machine, not how to avoid one.',
      ],
      materials: [
        { label: 'Closing plenary slides', url: 'https://docs.google.com/presentation/d/SAMPLE-REPLACE-ME/edit', presenter: 'Ted Hill' },
        { label: 'Symposium programme (PDF)', url: 'https://drive.google.com/file/d/SAMPLE-REPLACE-ME/view' },
      ],
      photoUrls: [
        'https://images.unsplash.com/photo-1540575467063-178a50c2df87?w=800&auto=format&fit=crop&q=80',
        'https://images.unsplash.com/photo-1505373877841-8d25f7d46678?w=800&auto=format&fit=crop&q=80',
        'https://images.unsplash.com/photo-1475721027785-f74eccf877e2?w=800&auto=format&fit=crop&q=80',
      ],
    },
  },
  {
    id: 'evt-cop-colloquium-2026',
    slug: 'communities-of-practice-colloquium-nov-2026',
    category: 'Community',
    name: 'Communities of Practice Colloquium',
    shortName: 'CoP Colloquium',
    tagline: 'Gathering the community voices',
    summary:
      'A morning for our communities of practice to think together, in one room, about a human-centred and mission-driven approach to AI - and what it asks of us next.',
    description:
      'Our communities of practice spend the year working in parallel. This colloquium puts them in the same room for a morning to compare what they are finding, surface where they disagree, and set a shared direction for a human-centred, mission-driven approach to AI at Chadwick.',
    startDate: '2026-11-28',
    endDate: '2026-11-28',
    dateLabel: 'Saturday 28 November 2026 · 9:00 AM - 12:00 Noon',
    venueName: 'Main Theater, Chadwick International',
    venueAddress: '45 Art center-daero 97beon-gil, Yeonsu-gu, Incheon, Republic of Korea',
    heroImageUrl: 'https://images.unsplash.com/photo-1475721027785-f74eccf877e2?w=1200&auto=format&fit=crop&q=80',
    registrationOpen: true,
    isTemplate: true,
    highlights: [
      { label: 'Format', value: 'Morning' },
      { label: 'Communities', value: '6' },
      { label: 'Capacity', value: '600' },
    ],
    about: [
      {
        heading: 'Everyone in one room',
        body: 'Communities of practice work well precisely because they are small and self-directed. The cost is that they rarely hear each other. This morning is the correction: each community reports what it has actually learned, including what did not work.',
      },
      {
        heading: 'Human-centred, by argument',
        body: 'The question is not whether to use AI but where the human has to stay in the loop, and why. We would rather leave with a disagreement named clearly than a consensus nobody believes.',
      },
    ],
    recap: {
      summary:
        'Each community of practice reported what it had actually learned across the year, including what did not work. The colloquium closed on a disagreement about AI marking that was deliberately left unresolved.',
      takeaways: [
        'Communities that published a short written account of their year were read; those that presented slides were not.',
        'The failures were the most-cited sessions in the feedback. Report them on purpose.',
        'A named disagreement travels further than a manufactured consensus.',
      ],
      materials: [
        { label: 'Colloquium proceedings', url: 'https://drive.google.com/file/d/SAMPLE-REPLACE-ME/view' },
      ],
    },
  },

  {
    id: 'evt-designing-agency-moe',
    slug: 'designing-for-agency-incheon-moe',
    category: 'Workshop',
    name: 'Designing for Agency: Workshop for Korean Educators',
    shortName: 'Designing for Agency',
    tagline: 'Human-centred and ethical AI, in practice',
    summary:
      'Delivered at the Incheon Metropolitan Office of Education, drawing on the Designing for AGENCY playbook - a working session on keeping student agency at the centre as AI enters the classroom.',
    description:
      'A full-day workshop for Korean educators at the Incheon Metropolitan Office of Education, built on the Designing for AGENCY playbook. Participants worked through the framework on their own units, testing where AI supports student agency and where it quietly removes it.',
    startDate: '2026-06-12',
    endDate: '2026-06-12',
    dateLabel: 'Friday 12 June 2026',
    venueName: 'Incheon Metropolitan Office of Education',
    heroImageUrl: 'https://images.unsplash.com/photo-1517245386807-bb43f82c33c4?w=1200&auto=format&fit=crop&q=80',
    registrationOpen: false,
    isTemplate: true,
    presenters: [
      {
        name: 'Nick Garvin',
        role: 'Workshop Lead, Chadwick International',
        note: 'Co-author of the Designing for AGENCY playbook.',
      },
    ],
    outcomes: [
      { label: 'Korean educators', value: '86' },
      { label: 'Schools', value: '31' },
      { label: 'Playbook', value: 'Designing for AGENCY' },
    ],
    recap: {
      summary:
        'Eighty-six Korean educators spent a day putting the AGENCY framework against units they already teach. The most useful hour was the one where nothing worked: three groups found that the AI step they had added removed exactly the decision the unit was designed to teach.',
      takeaways: [
        'Ask what decision the student is making. If a tool makes that decision, it has replaced the lesson rather than supported it.',
        'Agency is a design property of the task, not a setting in the software.',
        'Teachers moved faster when they redesigned one existing unit than when they planned a new one.',
      ],
      materials: [
        { label: 'Designing for AGENCY playbook', url: 'https://drive.google.com/file/d/SAMPLE-REPLACE-ME/view', presenter: 'Nick Garvin' },
        { label: 'Workshop slides (Korean and English)', url: 'https://docs.google.com/presentation/d/SAMPLE-REPLACE-ME/edit' },
      ],
    },
  },

  {
    id: 'evt-jim-knight-2027',
    slug: 'jim-knight-impact-cycle-2027',
    category: 'Workshop',
    name: 'The Impact Cycle with Jim Knight',
    shortName: 'Impact Cycle',
    tagline: 'Coaching that changes what happens in the room',
    summary:
      'An instructional coaching intensive on the Impact Cycle - identify, learn, improve - for coaches, mentors and anyone whose job includes helping a colleague get better.',
    description:
      'A two-day intensive on the Impact Cycle: helping a teacher identify a clear goal, learn a practice worth trying, and improve it through iteration. Built for instructional coaches, mentors, division heads and anyone whose role includes making a colleague more effective without taking over their classroom.',
    startDate: '2027-03-11',
    endDate: '2027-03-12',
    dateLabel: 'Thursday 11 - Friday 12 March 2027',
    venueName: 'Black Box, Chadwick International',
    heroImageUrl: 'https://images.unsplash.com/photo-1544717305-2782549b5136?w=1200&auto=format&fit=crop&q=80',
    registrationOpen: true,
    isTemplate: true,
    presenters: [
      {
        name: 'Jim Knight',
        role: 'Instructional Coaching Group',
        note: 'Author of The Impact Cycle and Better Conversations.',
      },
    ],
    highlights: [
      { label: 'Days', value: '2' },
      { label: 'Places', value: '200' },
      { label: 'Focus', value: 'Coaching' },
    ],
  },

  {
    id: 'evt-kath-murdoch-2027',
    slug: 'kath-murdoch-inquiry-2027',
    registrationOpensAt: '2027-01-15',
    category: 'Workshop',
    name: 'The Power of Inquiry with Kath Murdoch',
    shortName: 'Power of Inquiry',
    tagline: 'Learning that begins with a question',
    summary:
      'A practical workshop on inquiry-based learning - how to structure it, how to hold the uncertainty, and how to tell genuine inquiry from an activity wearing its clothes.',
    description:
      'A practical workshop on inquiry as a stance rather than a unit type. How to plan for it, how to sit with the uncertainty it creates, and how to tell the difference between genuine student inquiry and an activity that merely looks like it.',
    startDate: '2027-05-07',
    endDate: '2027-05-08',
    dateLabel: 'Friday 7 - Saturday 8 May 2027',
    venueName: 'Collaboration and Innovation Hub, Chadwick International',
    heroImageUrl: 'https://images.unsplash.com/photo-1503676260728-1c00da094a0b?w=1200&auto=format&fit=crop&q=80',
    registrationOpen: false,
    isTemplate: true,
    presenters: [
      {
        name: 'Kath Murdoch',
        role: 'Seastar Education',
        note: 'Author of The Power of Inquiry.',
      },
    ],
    highlights: [
      { label: 'Days', value: '2' },
      { label: 'Places', value: '40' },
      { label: 'Focus', value: 'Inquiry' },
    ],
  },
];

/* Seed feedback so the organiser report has something to aggregate. */
export const INITIAL_FEEDBACK: FeedbackEntry[] = [
  {
    id: 'fb-001', userId: 'usr-david-kim', targetKind: 'session', targetId: 'sess-01-keynote',
    rating: 5,
    glow: 'The framing of the mission as the question rather than the backdrop landed hard. I have been to a lot of AI keynotes and this is the first that started from what a school is for.',
    grow: 'It ran long enough that the Q&A got squeezed, and the questions were where the disagreement was.',
    submittedAt: 'Oct 15, 2027 10:40 AM', isAnonymous: false,
  },
  {
    id: 'fb-002', userId: 'usr-elena-rostova', targetKind: 'session', targetId: 'sess-01-keynote',
    rating: 4,
    glow: 'Genuinely useful to hear a school say plainly which parts of its mission it will not automate.',
    grow: 'Would have valued a concrete example of a decision that went the other way - something they tried and pulled back on.',
    submittedAt: 'Oct 15, 2027 11:02 AM', isAnonymous: true,
  },
  {
    id: 'fb-003', userId: 'usr-meiling-zhou', targetKind: 'overall', targetId: 'overall',
    rating: 5,
    glow: 'Tables grouped by mission strand at lunch was a small decision that changed the whole day. I spoke to people I would never have found.',
    grow: 'Two days is tight for five strands. I had to choose between sessions I wanted equally.',
    submittedAt: 'Oct 16, 2027 05:10 PM', isAnonymous: false,
  },
  {
    id: 'fb-004', userId: 'usr-chloe-bennett', targetKind: 'meal', targetId: 'catering',
    rating: 3,
    glow: 'Halal option was properly thought through, not an afterthought, and the labelling was clear.',
    grow: 'The queue at Day 1 lunch took most of the break. Two service points instead of one would fix it.',
    submittedAt: 'Oct 15, 2027 01:20 PM', isAnonymous: false,
  },
];
